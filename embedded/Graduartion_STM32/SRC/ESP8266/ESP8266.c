...
struct ESP_INIT_PARAM param = {1,"WCE","A2608r1996","GET /API/insert.php?UnitId=1&SoilTemp=35.6%20&SoilMoisture=30.214&AirTemp=30.2&AirHumidity=12.7 HTTP/1.1\r\nHOST:192.168.1.4\r\nConnection: close\r\n\r\n ",
"192.168.1.4" ,80,"TCP"};
...
uint8_t ESP_vidJoinNetwork ()
{
	...
	case 1:
		...
		ListenReturn =ESP8266_vidListen("OK",2,10000); // Time out in 5 s
		switch (ListenReturn)
		{
		case Found:
...
uint8_t ESP8266_u8SendGETDataToOpendConnection( )
{
	...
	copyString(command,"AT+CIPSEND=");
		...
		//SendGETListenR eturn = ESP8266_vidListen("OK",2,3000);
		dataSize=0;
		dataState++;
		break;
	/*
